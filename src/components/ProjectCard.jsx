...
const ProjectCard = ({ project, variant = 'archive' }) => {
  ...
  // Homepage variant — compact grid card
  if (variant === 'homepage') {
    return (
      <Link to={`/projects/${project.id}`} className="block group transition-transform duration-300 ease-out hover:translate-y-[-2px]">
        <div
          className="relative aspect-[16/10] rounded-xl overflow-hidden shadow-sm shadow-neutral-900/[0.05] ring-1 ring-neutral-900/[0.04] mb-3"
        ...
        <div>
          ...
          <h3 className="text-[0.9375rem] font-semibold leading-snug text-neutral-800 mb-1 group-hover:text-[#111111] transition-colors duration-200">
            {project.name}
          </h3>
          <p className="text-sm text-neutral-500 leading-relaxed line-clamp-1 mb-2.5">
            {project.tagline}
          </p>
  ...
  return (
    <Link to={`/projects/${project.id}`} className="block group transition-transform duration-300 ease-out hover:translate-y-[-2px]">
      <div className="relative aspect-[16/10] rounded-xl overflow-hidden shadow-sm shadow-neutral-900/[0.05] ring-1 ring-neutral-900/[0.04] mb-3">
        <img
          src={imageUrl}
          alt={project.name}
          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.03]"
      ...
      <div>
        ...
        <h3 className="text-[1.125rem] font-medium mb-1 group-hover:text-[#111111] transition-colors duration-200">
          {project.name}
        </h3>
